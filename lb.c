...
static __always_inline struct endpoint *get_next_backend() {
  ...
  if (!last_idx) return NULL;

  __u32 current_idx = __sync_fetch_and_add(last_idx, 1);

  __u32 target_idx = current_idx % NUM_BACKENDS;

  return bpf_map_lookup_elem(&backends, &target_idx);
}
// check the error type at https://github.com/torvalds/linux/blob/e774d5f1bc27a85f858bce7688509e866f8e8a4e/include/uapi/linux/bpf.h#L7300
  ...
  switch(rc) {
	  ...
	  case BPF_FIB_LKUP_RET_PROHIBIT: 
          ...
          case BPF_FIB_LKUP_RET_NOT_FWDED:
	  	  bpf_printk("FIB lookup failed: NOT_FORWARDED route. Destination might be on same subnet");
		  break;
          case BPF_FIB_LKUP_RET_FWD_DISABLED:
	  	  bpf_printk("FIB lookup failed: FORWARDING_DISABLED. Enable via sysctl");
		  break;
	  case BPF_FIB_LKUP_RET_UNSUPP_LWT:
	    	  bpf_printk("FIB lookup failed: UNSUPPORTED_PACKET. fwd requires encapsulation");
		  break;
          case BPF_FIB_LKUP_RET_NO_NEIGH:
	          bpf_printk("FIB lookup failed: NO NEIGHBOUR FOUND. Populate the cache with ping");
		  break;
	  case BPF_FIB_LKUP_RET_FRAG_NEEDED:
	  	  bpf_printk("FIB lookup failed: FRAGMENTATION NEEDED. Packet exceed MTU");
		  break;
	  case 	BPF_FIB_LKUP_RET_NO_SRC_ADDR:
	  	  bpf_printk("FIB lookup failed: NO SOURCE ADDR FOUND. Make sure source interface has legit ip");
		  break;
          default:
		  bpf_printk("FIB lookup failed: rc=%d (unknown). Check routing and ARP/NDP config", rc);
		  break;
  }
}
//see the bpf_fib_lookup struct in https://github.com/libbpf/libbpf/blob/dd92bef7f6c7a00bb0312554119b6d9cf38e4f32/include/uapi/linux/bpf.h#L7315
//see the xdp_md struct in https://github.com/libbpf/libbpf/blob/dd92bef7f6c7a00bb0312554119b6d9cf38e4f32/include/uapi/linux/bpf.h#L6559
static __always_inline int fib_lookup_v4_full(struct xdp_md *ctx, struct bpf_fib_lookup *fib, __u32 src, __u32 dst, __u16 tot_len) {
...
int xdp_loadbalancer(struct xdp_md *ctx) {
	 ...
	 * bpf_htons -> change the host byte order(based on host endian) to network byte order (big endian)	
	 * comparing value inside network packet with the host constant
	 */
	 ...
	 * simply rewriting the src and dst work but lb become bottleneck for handling both request and response
	 * client -> lb -> backend, backend -> lb -> client
	 * making upstream server return directly to client is ideal approach for high performance
	 * can only rewrite the dest ip and pass to backend 
	 * but client will drop response packet if req packet dst and res packet src ip are different
	 * without rewriting the dest or src ip but mac addr rewriting , need virtual ip to share for both lb and backends
	 * but backends and lb must be in the same layer 2 network
	 * use ipip encapsulation for lb and backend can exist in different layer 2 network
	*/

	/*
	 * make room for new outer ipv4 header (20bytes) between eth and inner ipv4 headers
	 * use negative delta for expending , since data-pointer move left , leaving more room
	*/
	...
	outer->ttl = 64;
	outer->protocol = IPPROTO_IPIP; //for kernel ipip module to check and proceed decapsulation
	...
	/*
	 * need to recalculate the layer3 checksum since we build the outer ip header from scratch
	 * since we are operating layer below kernel network stack,
	 * every changes in the packet headers need to recalculate the checksum 
	*/
	outer->check = recalc_ip_checksum(outer);
	
	/*
	 * we don't need to recalculate the ethernet frame checksum after updating the mac address
	 * bacause ethernet frame checksum (FCS) isn't in the header 
	 * but instead is automatically recomputed by the NIC hardware when packet is transmitted
	*/
	return XDP_TX;
}	
